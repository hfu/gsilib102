...
#define NINCDCB     262144
static int adddcbdata(nav_t *nav, const dcb_t *data)
{
	dcb_t *dcb_data;

	if (nav->ndmax<=nav->nd) {
		if (nav->ndmax<=0) nav->ndmax=NINCDCB; else nav->ndmax*=2;
		if (!(dcb_data=(dcb_t *)realloc(nav->dcb,sizeof(dcb_t)*nav->ndmax))) {
			trace(1,"adddcbdata: memalloc error n=%dx%d\n",sizeof(dcb_t),nav->ndmax);
			free(nav->dcb); nav->dcb=NULL; nav->nd=nav->ndmax=0;
            return -1;
        }
		nav->dcb=dcb_data;
    }
	...
	while (fgets(buff,sizeof(buff),fp)) {
		...
		if( sys > SYS_NONE){
			...
			if(stationname[0]!='\0')
			{
				...
				for(i=0;i<nav->nd;i++)
				{
					if(strcmp( nav->dcb[i].sta_name,stationname)==0){
						/*一致データあり バイアス情報のみ追加 */
						nav->dcb[i].cbias_s[s_code-1][type-1]=cbias*1E-9*CLIGHT;
						res=1;
						break;
					}
				}
				/*局未登録 新規追加*/
				if(res==0){
					strcpy(data.sta_name,stationname);
					data.cbias_s[s_code-1][type-1]=cbias*1E-9*CLIGHT;
					adddcbdata(nav,&data);
				}
			}
		}
