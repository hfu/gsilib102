...
static void writesolhead(stream_t *stream, const prcopt_t *proopt, const solopt_t *solopt)
{
    unsigned char buff[1024];
    int n;
    
	n=outsolheads(buff,proopt,solopt);
...
static void writesol(rtksvr_t *svr, int index)
{
    ...
    for (i=0;i<2;i++) {
        /* output solution */
		n=outsols(buff,&svr->rtk.sol,svr->rtk.rb,&svr->rtk.opt,svr->solopt+i);
    ...
    if (svr->moni) {
		n=outsols(buff,&svr->rtk.sol,svr->rtk.rb,&svr->rtk.opt,&solopt);
...
//	outsolcrinexrec(&svr->rtk.sol.time, "test", 1.0);
//	outrnxobsb_ion(const obsd_t *obs, const nav_t *nav, const double *ion, int n, int flag);
}
...
//		svr->rtcm[index].sta.pos[0] = svr->rtcm[1].sta.pos[i];
//		svr->rtcm[index].sta.pos[0] = svr->rtcm[1].sta.pos[i];
//		svr->rtcm[index].sta.pos[0] = svr->rtcm[1].sta.pos[i];
		if (index==0) {
		}
...
#endif
{
    ...
    for (cycle=0;svr->state;cycle++) {
		...
		for (i=0;i<fobs[0];i++) { /* for each rover observation data */
            ...
            /* rtk positioning */
            rtksvrlock(svr);
			rtkres=rtkpos(&svr->rtk,obs.data,obs.n,&svr->nav);
			rtksvrunlock(svr);
			...
			if(rtkres==1) {
				if ((svr->rtk.opt.mode==PMODE_SINGLE) || (svr->rtk.opt.mode==PMODE_PPP_KINEMA) ||
					(svr->rtk.opt.mode==PMODE_PPP_STATIC) || (svr->rtk.opt.mode==PMODE_PPP_FIXED)) {
					sprintf(staid, "%4d", svr->rtcm[0].staid);
					outsolcrinexrec(&obs.data[0].time,svr->rtcm[0].sta.name2,svr->rtk.sol.dtr[0]);
				}
			 //	if((svr->rtk.opt.mode!=PMODE_STATIC) && (svr->rtk.opt.mode!=PMODE_PPP_STATIC)) {
					outsolsnx(&svr->rtk, &obs);
			 //	}
			}
		}
					   ...
					   solopt_t *solopt, stream_t *moni)
{
	...
	if(!checkopts(prcopt,solopt,NULL))
	{
		return 0;
	}

	mode=PMODE_DGPS<=prcopt->mode&&prcopt->mode<=PMODE_FIXED;
	...
	/*1/4波長シフトテーブルを読み込み*/
	if(prcopt->phasshft==1){
		if((readL2C(prcopt->mopt.ifpcs,&(svr->nav)))!=0){
	...
	/*GLoNASS IFBテーブルを使用？*/
	if(prcopt->glomodear==GLO_ARMODE_IFB){
		/*IFBテーブルの読み込み*/
		if((readifb(prcopt->mopt.ififb,&(svr->nav)))!=0){
			showmsg("error : no GLOBASS IFB table file");
        	trace(1,"no GLOBASS IFB table file\n");
			return 0;
		}
	}
	/*観測誤差モデルを使用？*/
	if(prcopt->errmodel==1){
		/*観測誤差モデルの読み込み*/
		if((readerr(prcopt->mopt.iferr,&(svr->nav)))!=0){
    ...
    for (i=0;i<3;i++) { /* input/log streams */
        svr->nb[i]=svr->npb[i]=0;
		if (!(svr->buff[i]=(unsigned char *)malloc(svr->buffsize))||
		...
		/*L2C優先の場合、受信機がJavadに限り、受信機オプションに-GL2Cを追加する。*/
		if (prcopt->l2cprior==1 && svr->format[i]==STRFMT_JAVAD) {
			strcat(svr->raw[i].opt," -GL2C");
		}
    ...
    for (i=0;i<2;i++) { /* output peek buffer */
		if (!(svr->sbuf[i]=(unsigned char *)malloc(svr->buffsize))) {
            tracet(1,"rtksvrstart: malloc error\n");
            return 0;
        }
    }
	...
	// output files
	rtkopencrinex(prcopt,solopt,NULL,solopt->clcfile, &svr->rtcm[0].sta);
	rtkopensnx(prcopt,NULL,&svr->rtcm[0].sta,solopt->possinexfile, NULL, 0);


	/* DCB */
	for (i=0;i<(mode?2:1);i++) {
		setdcb(&svr->nav, &svr->rtcm[i].sta);
	}
	/* ISB */
//	for (i=0;i<(mode?2:1);i++) {
//		setisb(&svr->nav, svr->rtcm[i].sta.rectype, &svr->rtcm[i].sta);
//	}
	if(mode==1)	setisb(&svr->nav, svr->rtcm[0].sta.rectype, svr->rtcm[1].sta.rectype, &svr->rtcm[0].sta, &svr->rtcm[1].sta);
	else        setisb(&svr->nav, svr->rtcm[0].sta.rectype, svr->rtcm[1].sta.rectype, NULL             , NULL             );
                         ...
                         const solopt_t *solopt)
{
    ...
    if (index<=4) {
        ...
        writesolhead(svr->stream+index,&svr->rtk.opt,svr->solopt+index-3);
    }
    rtksvrunlock(svr);
    return 1;
}
