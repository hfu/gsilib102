...
*           2016/03/31  1.1  fix bug on reading galileo "C1" in rinex 2.12
*-----------------------------------------------------------------------------*/

#include "rtklib.h"

double interval=0;
unsigned char nav_sat[MAXSAT];
unsigned char obs_sat[MAXSAT];
unsigned char obs_sys[NUMSYS];
static const char rcsid[]="$Id:$";
int obstype[NUMSYS]={0};
char type[NUMSYS][MAXOBSTYPE][4]={{""}};
int repeatnum=0;
...
#ifdef ANTAPP
#define NINCOBS     26214              /* inclimental number of obs data */
#else
...
/* global variables ----------------------------------------------------------*/
char g_tobs[NUMSYS][MAXOBSTYPE][4];

static char targetcode='\0';
...
typedef struct {                        /* signal index type */
    ...
    int prpos[MAXFREQ][4][MAXCODE];     /*  */
    int l2pccodepos[2];                 /* L2P and L2C code index (GPS only) */
} sigind_t;
...
/*static*/extern int addobsdata(obs_t *obs, const obsd_t *data)
{
	obsd_t *obs_data;
	double diff;
    int a =sizeof(obsd_t);
	...
	if(obs->n>0){
		diff = settspan_d(data->time,obs->data[obs->n-1].time);
		if(interval <= 0 || (diff < interval && diff != 0)){
			if(fabs(diff - (int)(diff + 0.5))<= 0.1){
				interval = (int)(diff + 0.5);
			}
			else{
				interval = diff;
			}
	...
	obs->data[obs->n++]=*data;
	obs_sat[data->sat-1]=1;
	switch(satsys(data->sat,NULL)){
        case SYS_GPS: obs_sys[0]=1; break;
        case SYS_GLO: obs_sys[1]=1; break;
        case SYS_GAL: obs_sys[2]=1; break;
        case SYS_QZS: obs_sys[3]=1; break;
        case SYS_SBS: obs_sys[4]=1; break;
        case SYS_CMP: obs_sys[5]=1; break;
    }
...
static void convcode(double ver, int sys, const char *str, char *type)
{
    ...
    if      (!strcmp(str,"P1")) { /* ver.2.11 GPS L1PY,GLO L2P */
		...
		if (ver>=2.12){
			if      (sys==SYS_GPS) ; /* skip */
			else if (sys==SYS_GLO) ; /* skip */
			else if (sys==SYS_GAL) sprintf(type,"%c1X",'C');
			else if (sys==SYS_QZS) ; /* skip */
    ...
    if      (strstr(label,"MARKER NAME"         )) {
        ...
        for (j=nt=0,k=7;j<n;j++,k+=4) {
			...
			if (nt<MAXOBSTYPE-1){
				...
				strcpy(type[i][nt-1],tobs[i][nt-1]);
			}
        }
        *tobs[i][nt]='\0';
        ...
        for (i=nt=0,j=10;i<n;i++,j+=6) {
            ...
            if (ver<=2.99) {
				...
				strcpy(type[0][nt],str);
				strcpy(type[1][nt],str);
				strcpy(type[2][nt],str);
				strcpy(type[3][nt],str);
				strcpy(type[4][nt],str);
				strcpy(type[5][nt],str);
				obstype[0]=obstype[1]=obstype[2]=obstype[3]=obstype[4]=obstype[5]=n;
            }
            nt++;
        }
        *tobs[0][nt]='\0';
...
static void decode_navh(char *buff, nav_t *nav)
{
    ...
    if      (strstr(label,"ION ALPHA"           )) { /* opt ver.2 */
        ...
        if (nav) {
			...
			/*ŽžŒn•â³ƒpƒ‰ƒ[ƒ^GPGAiGPS to GALj‚ÆQZGP(QZS to GPS)‚ð’Ç‰Á“Ç‚Ýž‚ÞB*/
			else if(!strncmp(buff,"GPGA",4)) {
				j=5;
				nav->gps_gal[0] = str2num(buff,j,17);j+=17;
				nav->gps_gal[1] = str2num(buff,j,16);j+=16;
				nav->gps_gal[2] = (int)str2num(buff,j,7);j+=7;
				nav->gps_gal[3] = (int)str2num(buff,5,5);
			}
			else if(!strncmp(buff,"QZGP",4)) {
				j=5;
				nav->qzs_gps[0] = str2num(buff,j,17);j+=17;
				nav->qzs_gps[1] = str2num(buff,j,16);j+=16;
				nav->qzs_gps[2] = (int)str2num(buff,j,7);j+=7;
				nav->qzs_gps[3] = (int)str2num(buff,5,5);
			}
						  ...
						  obsd_t *obs)
{
    ...
    unsigned char llicode[MAXOBSTYPE]={0};
	char satid[8]="";
    ...
    for (i=0,j=ver<=2.99?0:3;i<ind->n;i++,j+=16) {
        ...
        if (stat) {
            ...
            llicode[i]=(unsigned char)str2num(buff,j+14,1)&4;
			obs->value[i]=val[i];
        }
    }
    if (!stat) return 0;
    ...
    for (i=0;i<NFREQ+NEXOBS;i++) {
        obs->P[i]=obs->L[i]=obs->SN[i]=0.0; obs->D[i]=0.0f;
        obs->SNR[i]=obs->LLI[i]=obs->LLIcode[i]=obs->code[i]=0;
    }
    obs->dPpc=0.0;
	...
	if(0==NEXOBS) {
		for (i=0;i<MAXFREQ;i++) {
			if((ind->prpos[i][0][0]==-1) && (ind->prpos[i][1][0]==-1)) {
				continue;
			}
			s=0;
			for (j=0;j<MAXCODE;j++) {
				if((ind->prpos[i][0][j]==-1) && (ind->prpos[i][1][j]==-1)) {
					break;
				}
				if(ind->prpos[i][0][j]!=-1) {
					if(val[ind->prpos[i][0][j]]!=0.0) {
						obs->P[i]=val[ind->prpos[i][0][j]];
						obs->code[i]=ind->code[ind->prpos[i][0][j]];
						obs->LLIcode[i]=llicode[ind->prpos[i][1][j]];
						s+=1;
					}
				}
				if(ind->prpos[i][1][j]!=-1) {
					if(val[ind->prpos[i][1][j]]!=0.0) {
						obs->L[i]=val[ind->prpos[i][1][j]];
						obs->LLI[i]=lli[ind->prpos[i][1][j]];
					//    s+=1;
					}
				}
				if(ind->prpos[i][2][j]!=-1) {
					if(val[ind->prpos[i][1][j]]!=0.0) {
						obs->D[i]=(float)val[ind->prpos[i][2][j]];
					//    s+=1;
					}
				}
				if(ind->prpos[i][3][j]!=-1) {
					if(val[ind->prpos[i][3][j]]!=0.0) {
						obs->SNR[i]=(unsigned char)(val[ind->prpos[i][3][j]]*4.0+0.5);
						obs->SN[i]=val[ind->prpos[i][3][j]];
					//    s+=1;
					}
				}
				if(s!=0) {
		...
		if((ind->l2pccodepos[0]!=-1) && (ind->l2pccodepos[1]!=-1)) {
			if((val[ind->l2pccodepos[0]]!=0.0) && (val[ind->l2pccodepos[1]]!=0.0)) {
				obs->dPpc = val[ind->l2pccodepos[0]] - val[ind->l2pccodepos[1]];
			}
		}
	}
	else {
                      ...
                      char tobs[MAXOBSTYPE][4], sigind_t *ind)
{
    ...
    double shift;
    int i,j,k,l,m,n;
    unsigned char code[MAXFREQ][MAXCODE] = {0};
    unsigned char pri[MAXFREQ][MAXCODE] = {0};
    unsigned char priL2[2] = {0};
    ...
    for (i=0;i<NFREQ;i++) {
		for (j=0,k=0;*tobs[j];j++) {
            ...
            if(ind->type[j] < 0  || ind->type[j] > 4) {
                continue;
            }
            if(((CODE_L2P == ind->code[j]) || (CODE_L2W == ind->code[j]))
                && (ind->type[j] == 0) && (ind->l2pccodepos[0] == -1) && (ind->pri[j] > priL2[0])) {
                ind->l2pccodepos[0]= j;
                priL2[0] = ind->pri[j];
            }
            else if(((CODE_L2S == ind->code[j]) || (CODE_L2L == ind->code[j]) || (CODE_L2X == ind->code[j]))
                && (ind->type[j] == 0) && (ind->l2pccodepos[1] == -1) && (ind->pri[j] > priL2[1])) {
                ind->l2pccodepos[1]= j;
                priL2[1] = ind->pri[j];
            }
            for (l=0;l<k;l++) {
                if(ind->pri[j] > pri[i][l]) {
                    for(m=k-1;m>=l;m--) {
                        ind->prpos[i][0][m+1] = ind->prpos[i][0][m];
                        ind->prpos[i][1][m+1] = ind->prpos[i][1][m];
                        ind->prpos[i][2][m+1] = ind->prpos[i][2][m];
                        ind->prpos[i][3][m+1] = ind->prpos[i][3][m];
                        pri[i][m+1] = pri[i][m];
                        code[i][m+1] = code[i][m];
                    }
                    ind->prpos[i][0][0] = -1;
                      ...
                      char tobs[][MAXOBSTYPE][4], obs_t *obs)
{
    ...
    while ((n=readrnxobsb(fp,opt,ver,tobs,&flag,data))>=0&&stat>=0) {
		...
		if(n!= 0 && timediff(time,data[0].time)==0.0){
            repeatnum++;
			continue;
		}
		time = data[0].time;
...
/* read rinex clock ----------------------------------------------------------*/
/*static*/ extern int readrnxclk(FILE *fp, const char *opt, int index, nav_t *nav)
{
    pclk_t *nav_pclk;
    gtime_t time;
    double data[2];
...
/* read rinex file -----------------------------------------------------------*/
/*static*/ extern int readrnxfp(FILE *fp, gtime_t ts, gtime_t te, double tint,
...
#else
    /* read rinex header */
    if (!readrnxh(fp,&ver,type,&sys,&tsys,g_tobs,nav,sta)) return 0;
#endif
...
#else
        case 'O': return readrnxobs(fp,ts,te,tint,opt,index,ver,tsys,g_tobs,obs);
#endif
...
/*static*/ extern int readrnxfile(const char *file, gtime_t ts, gtime_t te, double tint,
					   const char *opt, int flag, int index, char *type,
					   obs_t *obs, nav_t *nav, sta_t *sta)
{
    FILE *fp;
    ...
    for (i=0;i<ns;i++) {
		...
		for (j=0;j<opt->nobs[m];j++) {
            ...
            switch (opt->tobs[m][j][0]) {
				...
				case 'I': outrnxobsf(fp,1.2345,-1); break;
			}
		}
...
extern int outrnxnavb(FILE *fp, const rnxopt_t *opt, const eph_t *eph)
{
	...
	t.sec = eph->toc.sec;
	t.time = eph->toc.time;
	if(sys==SYS_CMP && eph->toes+14.0>=-1E9 && 1E9>=eph->toes+14.0){
		t.time -= 14.0;
	}
    time2epoch(t,ep);
...
* get observation types
* args   : int  sys         I   sys index
*          int  targetFreq  I   target freq (0:L1, 1:L2, 2:L5)
*          int  targetType  I   target type (0:C(pseudorange) ,1:L(carrier-phase))
*          char **ret       O   observation types
* return : number of types
*-----------------------------------------------------------------------------*/
extern int get_tobs(int sys, int targetFreq, int targetType, char ret[][4])
{
    int i, num = 0, type, freq;
    for (i = 0; i < MAXOBSTYPE; i++) {
        if (strcmp(g_tobs[sys][i], "") == 0) {
            /* skip */
            continue;
        }

        /* get type */
        type=decode_obstype(g_tobs[sys][i],&freq);
        if (freq!=targetFreq) continue;

        /* add */
        if (type == targetType) {
            strcpy(ret[num], g_tobs[sys][i]);
            num++;
        }
    }
    return num;
...
/* set tareget types ------------------------------------------------
* set tareget types(pseudorange)
* args   : char  code  I   target code(pseudorange) ['\0':no target]
*-----------------------------------------------------------------------------*/
extern void set_targetcode(char code) {
    targetcode = code;
}
