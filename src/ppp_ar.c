/*------------------------------------------------------------------------------
* ppp_ar.c : ppp ambiguity resolution (GRG)
*
...
#define IS(sys,type,f,opt) (NP(opt)+NC(opt)+NT(opt)+NS0(opt)*(f)+((opt)->isb==ISBOPT_EST?(NSYS-NSYSGPS)*(type):0)+(sys-NSYSGPS))   /* isb */
                   ...
                   const double *NC, int n)
{
	...
	if ((info=filter(rtk->x,rtk->P,H,v,R,rtk->nx,n,rtk->ux))) {
                  ...
                  const double *azel)
{
	...
	if (n<=0||rtk->opt.ionoopt!=IONOOPT_IFLC||rtk->opt.nfreq<2) return 0;
	...
	if (rtk->opt.modepppar==PPPAR_CNES) {
		stat=fix_amb_ROUND(rtk,sat1,sat2,NW,m);
    }
    else if (rtk->opt.modepppar==PPPAR_CNES_ILS) {
