...
*           2015/07/29  1.1  update leap seconds
*-----------------------------------------------------------------------------*/
...
const prcopt_t prcopt_default={ /* defaults processing options */
    PMODE_SINGLE,0,0,           /* mode,soltype,nfreq */
    {NOSELECT,NOSELECT,NOSELECT,NOSELECT,NOSELECT,NOSELECT},
    SYS_GPS,                    /* navsys */
    15.0*D2R,{{0,0}},           /* elmin,snrmask */
    0,0,0,1,1,5,0,10,           /* sateph,modertkar,modepppar,modear,glomodear,maxout,minlock,minfix */
    0,0,0,0,                    /* estion,esttrop,dynamics,tidecorr */
    1,0,0,0,0,                  /* niter,codesmooth,intpref,sbascorr,sbassatsel */
    0,0,                        /* rovpos,refpos */
    {100.0,100.0},              /* eratio[] */
    {100.0,0.003,0.003,0.0,1.0}, /* err[] */
	{30.0,0.03,0.3,0.01,100.0,100.0,100.0}, /* std[] */
    ...
    {0},     /* positioning options */
    0,       /* solution sync mode (0:off,1:on) */
    {{0},{0}}, /* ocean tide loading parameters {rov,base} */
    {0},    /* extended receiver error model */
    
    
	0,/* L2 code priority (0:L2P,1:L2C) */
    {{""}}, /* code priority */
	0,/* L2 code priority (0:L2P,1:L2C) */
	0,/* time system correction ( 0:off,1:on) */
	0,/* phase cycle shift (0:off,1:table) */
	0,/* error model (0:user settings,1:table) */
    {0},/* code error ratio(no DCB) */
    {{0},{0}}, /* receiver types {rover,base} */

    0,     /* estimate PCV mode */
    0,     /* estimate PCV freq */
    {0,0},  /* estimate PCV {order, degree} */
    {0},   /* estimate PCV sphharmonic var */
	0,

    0,        /* GPS L2C/L2P bias (0:off,1:table,2:est) */
    0,            /* inter-system bias (0:off,1:table,2:est) */
    0,           /* differential method (0:inall,1:exc-glo) */
    1,           /* glonass tine offset estimation (0:off,1:est) */
    0,         /* pntpos chi-square test (0:off,1:on) */
	900,			/* NL-FCB区間間隔（秒） */
    {0}      /* Options-Setting3ダイアログの設定値 */
};
...
const solopt_t solopt_default={ /* defaults solution output options */
	...
	0,0,"",0,"",0,0,0,"",       /* sbres file,ion param, trop param,receiver clock,satellite clock output  */
    "", /* 基地局リスト（複数基線解析） */
	"", /* 移動局リスト（複数基線解析） */
	0,  /* FCB推定結果出力フラグ（複数基線解析） */
	""  /* FCB推定結果出力パス（複数基線解析） */
};
...
const char *systemstrs[]={          /* gnss system strings */
    "NOSYSYTEM"
    ,"GPS"
#ifdef ENAGLO
    ,"GLONASS"
...
const int   freqs[]={                   /* frequency */
	1,2,5,6,7,8,0
};
const char *freqstrs[]={          /* frequency strings */
    "L1","L2","L5","L6","L7","L8",NULL
};
...
unsigned char obsfreqs[]={ /* 1:L1,2:L2,3:L5,4:L6,5:L7,6:L8,7:L3 */
...
extern char codepris[6][MAXFREQ][MAXFCODE+1]={  /* code priority table */
...
char hisstation[50][5]={0};
const mbsopt_t mbsopt_default={
	0,0,                        /* estsatclk estsatfcb */
	"","","","","","","",       /* ifpcs ififb iferr ifsdp iffcb ofdir eptmp */
	7200.0,43200.0,{1E-4,1E-4,1E-7},          	/* tiztd tigra sigtrop[] */
	0.0,0.0,0.0,0.0,        	/* minpass minddpass maxsigw minconfw minconf1 */
	5.0,5.0,0.99990,0.99990,0.001,    	/* cpomcth promcth wlddfix l1ddfix itrconv */
	1,{0.0},{0.0}               /* itrmax sigr[] sigb[]*/
};
int duplnum=0;
...
/*  -----------------------------------------------------
* 観測誤差テーブル検索
* args   : char *rectype      I   受信機タイプ
*          char *anttype      I   アンテナタイプ
*          int sys            I   衛星システム番号
*          unsigned char code I   観測コード
*          int type           I   0:phase,1:code
*          nav_t *nav         I   航法データ
*          double *a          I/O 観測誤差モデルa
*          double *b          I/O 観測誤差モデルb
* return : int 			      0:観測誤差モデル不明(a,bの値変更なし)、1:観測誤差モデル更新
*-----------------------------------------------------------------------------*/
extern int search_errtbl(const char *rectype, const char *anttype, const int sys, const unsigned char code, const int type,
    ...
    static const char type_str[2] = {'L', 'C'};
    const err_t *err_tbl = nav->errs;
    char obstype[4];
    char *obsstr;
    int i, idx=-1 ,ret=0;
	char* p;
	...
	trace(4,"search: rectype=%s,anttype=%s\n",rectype,anttype);
    /* 観測誤差モデル係数：a,bの検索 */
	for(i = 0; i < nav->nerrs; i++){
		trace(4,"errtbl: rectype=%s,anttype=%s\n",err_tbl[i].rectype,err_tbl[i].anttype);
		if(strcmp(rectype, err_tbl[i].rectype)) {
            /* 受信機タイプが等しくない場合 */
            continue;
        }
//		if (strcmp(anttype, err_tbl[i].anttype)) {
		p=strstr(anttype, err_tbl[i].anttype);
		if (p==NULL) {
            /* アンテナタイプが等しくない場合 */
            continue;
        }
    ...
    /* 観測誤差モデルテーブル用の観測コードを取得 */
	obsstr = code2obs(code,NULL);
	sprintf(obstype, "%c%s",type_str[type], obsstr);

    /* 同じシステム/観測コードを検索する */
    for(i=0; i<MAXERRDAT; i++){
        /* 衛星システムの比較 */
        if(err_tbl[idx].sys[i] != sys){
            continue;
        }

        /* 観測コードの比較*/
        if(!strcmp(obstype, err_tbl[idx].sig[i])) {
			trace(4,"a and b renew\n");
            *a=err_tbl[idx].errvar[i][0];
            *b=err_tbl[idx].errvar[i][1];
            ret=1;
            break;
        }
	}
...
*          unsigned char code I   観測コード
*          nav_t *nav         I   航法データ
*          double *val        O   DCB補正値(NULL：戻り値なし)
* return : int                0：無、1:有
*-----------------------------------------------------------------------------*/
...
extern char checkCode(const int syscode, const int ifreq, char* ch) {
	...
	int i;
	char ret=0x00;
	if     (syscode==SYS_GPS) isys=0;
	else if(syscode==SYS_GLO) isys=1;
	else if(syscode==SYS_GAL) isys=2;
	else if(syscode==SYS_QZS) isys=3;
	else if(syscode==SYS_SBS) isys=4;
	else if(syscode==SYS_CMP) isys=5;
	else return ret;

	if(0x08 == *ch || 0x7F == *ch) {
		ret = *ch;
	}
	else if(0x20 > *ch || 0x7E < *ch) {
		ret = 0x00;
	}
	else {
		for(i=0;i<MAXCODE;++i) {
			*ch=toupper(*ch);
			if('\0' == codepris[isys][ifreq][i]) {
				break;
			}
			else if(*ch == codepris[isys][ifreq][i]) {
				ret = *ch;
				break;
			}
		}
...
/* integer zero matrix -----------------------------------------------------------------
* generate new integer zero matrix
* args   : int    n,m       I   number of rows and columns of matrix
* return : matrix pointer (if n<=0 or m<=0, return NULL)
*-----------------------------------------------------------------------------*/
...
static int filter_(const double *x, const double *P, const double *H,
                   ...
                   double *xp, double *Pp)
{
	double *F=zeros(n,m),*Q=zeros(m,m),*K=zeros(n,m),*I=eye(n);
	int info;
	double *dummy=zeros(n,1);
	double *tes=zeros(n,1);
				  ...
				  const double *R, int n, int m, const int *ux)
{
	...
	x_=zeros(k,1); xp_=zeros(k,1); P_=zeros(k,k); Pp_=zeros(k,k); H_=zeros(k,m);
	for (i=0;i<k;i++) {
		x_[i]=x[ix[i]];
	...
	trace(5,"P_=\n"); tracemat(5,P_,k,k,8,3);
	trace(5,"H_=\n"); tracemat(5,H_,k,m,8,3);
...
extern void imatfprint(const int A[], int n, int m, FILE *fp)
{
	...
	for (i=0;i<n;i++) {
		for (j=0;j<m;j++) fprintf(fp," %d",A[i+j*n]);
		fprintf(fp,"\n");
	}
}
/* string to number ------------------------------------------------------------
...
/* get current PC time in utc -----------------------------------------------------
* get current PC time in utc
* args   : none
* return : current PC time in utc
*-----------------------------------------------------------------------------*/
...
static int readngspcv(const char *file, pcvs_t *pcvs)
{
	...
	while (fgets(buff,sizeof(buff),fp)) {
        ...
        if (++n==1) {
			...
			/*NGS PCVANTEXのデータレコードの最初の行から、新たに日付を読み込んで、pcv_t構造体へ格納する。*/
			strncpy(pcv.date,buff+72,8); pcv.date[9]='\0';
        }
        else if (n==2) {
            if (decodef(buff,3,neu)<3) continue;
			pcv.off_rcv[0][0][0]=neu[1];
			pcv.off_rcv[0][0][1]=neu[0];
			pcv.off_rcv[0][0][2]=neu[2];
		}
		else if (n==3) decodef(buff,10,pcv.var_rcv[0][0]);
		else if (n==4) decodef(buff,9,pcv.var_rcv[0][0]+10);
		else if (n==5) {
			if (decodef(buff,3,neu)<3) continue;;
			pcv.off_rcv[0][1][0]=neu[1];
			pcv.off_rcv[0][1][1]=neu[0];
			pcv.off_rcv[0][1][2]=neu[2];
		}
		else if (n==6) decodef(buff,10,pcv.var_rcv[0][1]);
		else if (n==7) {
			decodef(buff,9,pcv.var_rcv[0][1]+10);
			for(i=1;i<NSYS;i++)
			{
				memcpy(pcv.off_rcv[i][0],pcv.off_rcv[0][0],sizeof(pcv.off_rcv[0][0]));
				memcpy(pcv.off_rcv[i][1],pcv.off_rcv[0][1],sizeof(pcv.off_rcv[0][1]));
				memcpy(pcv.var_rcv[i][0],pcv.var_rcv[0][0],sizeof(pcv.var_rcv[0][0]));
				memcpy(pcv.var_rcv[i][1],pcv.var_rcv[0][1],sizeof(pcv.var_rcv[0][1]));
			}
			addpcv(&pcv,pcvs);
        }
    }
...
extern char* monthnum(char* monthname){
	...
	int i;
	for(i=1;i<13;i++){
		if(strcmp(monthname,abbreviated_name[i])==0){
			return month[i];
		}
	}
	return NULL;
...
static int readantex(const char *file, pcvs_t *pcvs)
{
    ...
    pcv_t pcv;
    double neu[3];
	int i,j,f,freq=0,state=0;
	int sys;
	char sysc;
	char buff[256];
	char month[4];
	int flg_var[NSYS][NFREQ]={0},flg_off[NSYS][NFREQ]={0},flg0[NSYS][NFREQ]={0};
	...
	while (fgets(buff,sizeof(buff),fp)) {
		...
		if (strstr(buff+60,"START OF ANTENNA")) {
			memcpy(flg_off,flg0,sizeof(flg0));
			memcpy(flg_var,flg0,sizeof(flg0));
			pcv=pcv0;
			state=1;
		}
        ...
        if (strstr(buff+60,"TYPE / SERIAL NO")) {
			...
			if (sscanf(buff+3,"%c",&sysc)<1) continue;
			else sys=syschar2sysind(sysc);
			if (sys<1 ) continue;
			else pcv.sys = sysno(sys);
			...
			if(pcv.sat!=0)
			{
				pcv.off[freq-1][0]=neu[1]; /* x or e */
				pcv.off[freq-1][1]=neu[0]; /* y or n */
				pcv.off[freq-1][2]=neu[2]; /* z or u */
			}
			/* Receiver antenna */
			else if(pcv.sys!=0)
			{
				pcv.off_rcv[sys-1][freq-1][0]=neu[1]; /* x or e */
				pcv.off_rcv[sys-1][freq-1][1]=neu[0]; /* y or n */
				pcv.off_rcv[sys-1][freq-1][2]=neu[2]; /* z or u */
				flg_off[sys-1][freq-1]=1;
				/* GPS以外の衛星システムの"NORTH / EAST / UP"値がない場合、 */
				/* GPSの値を他システムにも適用する                          */
				if(pcv.sys==SYS_GPS)
				{
					for(i=1;i<NSYS;i++)
					{
						if(flg_off[i][freq-1]!=1)
						{
							pcv.off_rcv[i][freq-1][0]=neu[1]; /* x or e */
							pcv.off_rcv[i][freq-1][1]=neu[0]; /* y or n */
							pcv.off_rcv[i][freq-1][2]=neu[2]; /* z or u */
						}
					}
				}
			}
			...
			{
				if ((i=decodef(buff+8,19,pcv.var_rcv[sys-1][freq-1]))<=0) continue;
				for (;i<19;i++) pcv.var_rcv[sys-1][freq-1][i]=pcv.var_rcv[sys-1][freq-1][i-1];
				flg_var[sys-1][freq-1]=1;
				/* GPS以外の衛星システムの"NOAZI"値がない場合、 */
				/* GPSの値を他システムにも適用する              */
				if(pcv.sys==SYS_GPS)
				{
					for(i=1;i<NSYS;i++)
					{
						if(flg_var[i][freq-1]!=1)
						{
							memcpy(pcv.var_rcv[i][freq-1],pcv.var_rcv[sys-1][freq-1],sizeof(pcv.var_rcv[sys-1][freq-1]));
						}
					}
				}
		...
		/*ANTEXファイルの "METH / BY / # / DATE" キーワードの行を新たに読み込んで、日付を抽出してpcv_t構造体へ格納する。*/
		else if (strstr(buff+60,"METH / BY / # / DATE")) {
			/*dd-mmm-yyをyy/mm/ddへ文字列変換する処理*/
			strncpy(pcv.date,buff+57,2);
			strcpy(pcv.date+2,"/");
			strncpy(month,buff+53,3);
			month[3]='\0';
			strcpy(pcv.date+3,monthnum(month));
			strcpy(pcv.date+5,"/");
			strncpy(pcv.date+6,buff+50,2);
		}
    }
	fclose(fp);
...
{
    if (opt&0x01 && nav->eph) {free(nav->eph ); nav->eph=NULL; nav->n =nav->nmax =0;}
    if (opt&0x02 && nav->geph) {free(nav->geph); nav->geph=NULL; nav->ng=nav->ngmax=0;}
    if (opt&0x04 && nav->seph) {free(nav->seph); nav->seph=NULL; nav->ns=nav->nsmax=0;}
    if (opt&0x08 && nav->peph) {free(nav->peph); nav->peph=NULL; nav->ne=nav->nemax=0;}
    if (opt&0x10 && nav->pclk) {free(nav->pclk); nav->pclk=NULL; nav->nc=nav->ncmax=0;}
    if (opt&0x20 && nav->alm) {free(nav->alm ); nav->alm=NULL; nav->na=nav->namax=0;}
    if (opt&0x40 && nav->tec) {free(nav->tec ); nav->tec=NULL; nav->nt=nav->ntmax=0;}
...
{
	if (!fp_trace||level>level_trace) return;
	imatfprint(A,n,m,fp_trace); fflush(fp_trace);
}
				   ...
				   const char *base)
{
    ...
    if (time.time!=0) {
		...
		sprintf(rep,"%d",    (int)ep[3]/3+1);     stat|=repstr(rpath,"%ia",rep);
    }
...
static double interpvar(double ang, const double *var, double dzen)
{
    double a=ang/dzen; /* ang=0-90 */
    int i=(int)a;
...
*          int    sys       I   satellite system (SYS_GPS,SYS_GLO,... )
					 ...
					 int opt, double *dant)
{
	...
	for (i=0;i<NFREQ;i++) {
		for (j=0;j<3;j++) off[j]=pcv->off_rcv[idx][i][j]+del[j];

		dant[i]=-dot(off,e,3)+(opt?interpvar(90.0-azel[1]*R2D,pcv->var_rcv[idx][i],5.0):0.0);
	}
...
extern void antmodel_s(const pcv_t *pcv, double nadir, double *dant)
{
    ...
    for (i=0;i<NFREQ;i++) {
		dant[i]=interpvar(nadir*R2D,pcv->var[i],1.0);
    }
...
{
    if(sysind<1)             return 0;
	else if(ISYSGPS==sysind) return SYS_GPS;
    else if(ISYSGLO==sysind) return SYS_GLO;
    else if(ISYSQZS==sysind) return SYS_QZS;
    else if(ISYSGAL==sysind) return SYS_GAL;
	else if(ISYSCMP==sysind) return SYS_CMP;
	else if(ISYSSBS==sysind) return SYS_SBS;
...
{
	if(sysind<1)             return 0;
	else if(ISYSGPS==sysind) return 'G';
	else if(ISYSGLO==sysind) return 'R';
	else if(ISYSQZS==sysind) return 'J';
	else if(ISYSGAL==sysind) return 'E';
	else if(ISYSCMP==sysind) return 'C';
	else if(ISYSSBS==sysind) return 'S';
...
{
	if('G'==syschar)return ISYSGPS;
	else if('R'==syschar) return ISYSGLO;
	else if('J'==syschar) return ISYSQZS;
	else if('E'==syschar) return ISYSGAL;
	else if('C'==syschar) return ISYSCMP;
	else if('S'==syschar) return ISYSSBS;

	else return 0;
}

int isL2C(const int code)
{
	int l2c=0;
...
int isL2P(const int code)
{
	int l2p=0;
	if (code==CODE_L2P || code==CODE_L2W) l2p=1;
    return l2p;
}
...
extern double p_gamma(double a, double x, double log_gamma_a)
{
    double y,w;
    int i;
...
extern double q_gamma(double a, double x, double log_gamma_a)
{
...
/*曜日略称を数値に変換*/
int monthint(char* monthname){
	char *abbreviated_name[]={
		"","JAN","FEB","MAR","APR","MAY","JUN","JUL","AUG","SEP","OCT","NOV","DEC",""
	};
	int i;
	for(i=1;i<13;i++){
		if(strcmp(abbreviated_name[i],monthname)==0){
			return i;
		}
	}
	return 0;
...
/*曜日を英字3文字の略称に変換*/
void monthchange(char* monthname, struct tm *t){
	char *abbreviated_name[]={
		"","JAN","FEB","MAR","APR","MAY","JUN","JUL","AUG","SEP","OCT","NOV","DEC",""
	};
	strcpy(monthname,abbreviated_name[t->tm_mon+1]);
}
 ...
 * 関数名： check_intdd
 * 概要  ： 整数化判定
 *   IN  ： ddfix double FIX判定値
 *			val double 二重差
 *			ddint double 近傍の整数
 *			var double 二重差の分散
 *   OUT ： int ステータス(0:未,1:整数化完了)
 */
int check_intdd( double ddfix, double val, double ddint, double var, double *p0 )
{
//	double p0 = 1.0;
	double diff, denom;
	int i;
	static const int maxint=5;
	...
	*p0 = 1.0;
	...
	for( i = 1; i < maxint; i++ ) {
		if( var ) {
			*p0 -= ( erfc(( i - diff ) / denom ) - erfc(( i + diff ) / denom ));
		}
	}
	return ( *p0 > ddfix ) ? 1 : 0;
}
...
/* 相補誤差計算 */
/* http://www.opensource.apple.com/source/ruby/ruby-75.1/ruby/missing/erf.c */
double erfc( double x ) {
	return( x >= 0.0 ? q_gamma( 0.5, x * x, LOG_PI / 2.0 ) : 1.0 + p_gamma( 0.5, x * x, LOG_PI / 2.0 ));
}

int strtrm2num(char* str) {
	int val=0;
	int i;
	while(str[i]='\0') {
