...
int __fastcall TPlot::FixSerachTop(solbuf_t SolData, int *count)
{
	int i;
	sol_t *sol;

	for(i=0;i<SolData.n;i++){
		sol=SolData.data;
		if(sol[i].stat==SOLQ_FIX){
			*count=i;
			return 1;
		}
	}
...
int __fastcall TPlot::FixSerachBottom(solbuf_t SolData, int *count)
{
	int i;
	sol_t *sol;

	for(i=SolData.n-1;i>-1;i--){
		sol=SolData.data;
		if(sol[i].stat==SOLQ_FIX){
            *count=i;
			return 1;
...
int __fastcall TPlot::FixSerach(solbuf_t SolData, int *count, int lr)
{
	if(lr==1){
		return FixSerachTop(SolData,count);
	}else{
		return FixSerachBottom(SolData,count);
	}
}
...
void __fastcall TPlot::DrawTrk(int level)
{
    ...
    int i,j,index,sel=!BtnSol1->Down&&BtnSol2->Down?1:0,p=0,fixserach,fixflag=0;
    ...
    if (BtnSol1->Down) {
        ...
        DrawTrkStat(pos,header,p++,0);
        header="";
        if(!strcmp(posmode[0],"static")){
            fixflag=FixSerach(SolData[0],&fixserach,lastresult[0]);
            if(fixflag==1){
            pos=SolToPos(SolData,fixserach,0,0);
    ...
    if (BtnSol2->Down) {
        ...
        DrawTrkStat(pos,header,p++,1);
		if(!strcmp(posmode[1],"static")){
		    fixflag=FixSerach(SolData[1],&fixserach,lastresult[1]);
			if(fixflag==1){
		        pos=SolToPos(SolData+1,fixserach,0,0);
			    GraphT->ToPoint(pos->x[0],pos->y[0],p1);
				GraphT->DrawMark(p1,0,CColor[0],MarkSize*2+12,0);
...
void __fastcall TPlot::DrawTrkStat(const TIMEPOS *pos, AnsiString header, int p, int num)
{
    AnsiString s[6];
    TPoint p1,p2;
    ...
    if (pos->n>0) {
		...
		if(strcmp(posmode[num],"\0")){
			s[n++].sprintf("pos mode=%s",posmode[num],20);
		}
		delete [] d;
    }
	GraphT->GetPos(p1,p2);
...
void __fastcall TPlot::DrawSol(int level, int type)
{
    ...
    int i,j,k,sel=!BtnSol1->Down&&BtnSol2->Down?1:0,p=0,fixserach,fixflag=0;;
    ...
    if (BtnSol1->Down) {
        ...
        DrawSolStat(pos,unit[type],p++,0);

		if(!strcmp(posmode[0],"static")){
			fixflag=FixSerach(SolData[0],&fixserach,lastresult[0]);
			if(fixflag==1){
				pos =SolToPos(SolData+sel,fixserach,0,type);
				pos1=SolToPos(SolData    ,fixserach,0,type);
				pos2=SolToPos(SolData+1  ,fixserach,0,type);
...
void __fastcall TPlot::DrawSolStat(const TIMEPOS *pos, AnsiString unit, int p, int num)
{
    TSpeedButton *btn[]={BtnOn1,BtnOn2,BtnOn3};
    TPoint p1,p2;
    ...
    for (i=0;i<3;i++) {
        ...
        u=unit.c_str();
		if(strcmp(posmode[num],"\0")){
			s.sprintf("pos mode=%s AVE=%.4f%s STD=%.4f%s RMS=%.4f%s",posmode[num],ave,u,std,u,rms,u);
		}else{
    ...
    if (n>0&&((sel==0&&BtnSol1->Down)||(sel==1&&BtnSol2->Down))) {
        ...
        for (int sat=1;sat<=MAXSAT;sat++) {
            ...
            for (int i=0;i<n;i++) {
				...
				if (p->frq==2) {
					if     (1==cp&&!isL2P(p->code)) continue;
					else if(0==cp&&!isL2C(p->code)) continue;
				}
				if (p->resp==0.0&&p->resc==0.0) continue;
		  //      if (p->code!=20) continue;
                x[m]=TimePos(p->time);
                y[0][m]=p->resp;
                y[1][m]=p->resc;
                y[2][m]=p->el*R2D;
            ...
            for (int i=0;i<3;i++) {
                ...
                if (level&&PlotStyle<2) {
                    TColor color;
                    for (int j=0;j<m;j++) {
                        color=i<2?MColor[sel][q[j]]:MColor[0][1];
                ...
                if (level&&i==1) { /* slip */
                    for (int j=0;j<m;j++) {
                        if (!s[j]) continue;
                        TColor color=s[j]&1?MColor[sel][5]:MColor[0][0];
