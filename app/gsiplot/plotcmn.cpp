...
TIMEPOS * __fastcall TPlot::SolToPos(solbuf_t *sol, int index, int qflag, int type)
{
	...
	double vtmp[3], atmp[3];
    int i;
    
    trace(3,"SolToPos: n=%d\n",sol->n);

	pos=new TIMEPOS(index<0?sol->n:3,1);
	vel=new TIMEPOS(index<0?sol->n:3,1);
	acc=new TIMEPOS(index<0?sol->n:3,1);

	//
	if (index>=0) {
		data=getsol(sol,index<0?0:index);
		if(data==NULL || (data!=NULL && data->rva!=1)){
	...
	for (i=index<0?0:index;data=getsol(sol,i);i++) {
		...
		if(data->rva==1) //posmode = rva
		{
			vtmp[0]=data->rr[3]+OPos[0];
			vtmp[1]=data->rr[4]+OPos[1];
			vtmp[2]=data->rr[5]+OPos[2];
			PosToXyz(data->time,vtmp/*data->rr+3*/,data->type,xyz);
			CovToXyz(data->rr,data->qr+6,data->type,xyzs);
			...
			vel->n++;
			atmp[0]=data->rr[6]+OPos[0];
			atmp[1]=data->rr[7]+OPos[1];
			atmp[2]=data->rr[8]+OPos[2];
			PosToXyz(data->time,atmp/*data->rr+6*/,data->type,xyz);
			CovToXyz(data->rr,data->qr+12,data->type,xyzs);
			acc->t  [acc->n]=data->time;
			acc->x  [acc->n]=xyz [0];
			acc->y  [acc->n]=xyz [1];
			acc->z  [acc->n]=xyz [2];
			acc->xs [acc->n]=xyzs[0];
			acc->ys [acc->n]=xyzs[1];
			acc->zs [acc->n]=xyzs[2];
			acc->xys[acc->n]=xyzs[3];
			acc->q[acc->n]=data->stat;
			acc->n++;
		}
		pos->n++;
